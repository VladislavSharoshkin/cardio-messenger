...
import 'package:cardio_messenger/services/get_messages.dart';
...
import 'package:cardio_messenger/pages/chat/chat_view.dart';
import 'package:cardio_messenger/pages/chat/event_info_dialog.dart';
import 'package:cardio_messenger/pages/chat/recording_dialog.dart';
import 'package:cardio_messenger/utils/matrix_sdk_extensions.dart/event_extension.dart';
import 'package:cardio_messenger/utils/matrix_sdk_extensions.dart/ios_badge_client_extension.dart';
...
import 'package:cardio_messenger/utils/voip/callkeep_manager.dart';
...
class ChatController extends State<Chat> {
  ...
  bool get canSaveSelectedEvent =>
      selectedEvents.length == 1 &&
      {
        ...
        MessageTypes.Audio,
        MessageTypes.File,
      }.contains(selectedEvents.single.type);

  // void saveSelectedEvent() => selectedEvents.single.saveFile(context);

  List<MMessage> selectedEvents = [];
  ...
  final int _loadHistoryCount = 10;
  ...
  void initState() {
    ...
    SocketHelper.shared.socket.on('update', (data) => update());
    SocketHelper.shared.socket.on('logout', (data) => logout());
    super.initState();
  }
  ...
  void unfold(int eventId) {
    var i = filteredEvents!.indexWhere((e) => e.id == eventId);
    // setState(() {
  ...
  update() async {
    var getChat = await GetChat.get(int.parse(roomId!));
    var messages = await GetMessages.getMessages(int.parse(roomId!), null, _loadHistoryCount);
    setState(() {
      chat = getChat;
      filteredEvents = messages;
    });
  }
  ...
  Future<bool> getTimeline() async {
    chat ??= await GetChat.get(int.parse(roomId!));

    filteredEvents ??= await GetMessages.getMessages(int.parse(roomId!), null, _loadHistoryCount);
    setReadMarker();
    return true;
  }
  ...
  void setReadMarker([_]) {
    ...
    // }
    if (_setReadMarkerFuture == null && filteredEvents != null &&
        filteredEvents!.isNotEmpty && filteredEvents!.first.readState == 1) {
      Logs().v('Set read marker...');
      // ignore: unawaited_futures
      _setReadMarkerFuture = GetMessages.readMessage(int.parse(roomId!), filteredEvents!.first.id).then((_) {
        _setReadMarkerFuture = null;
      });
      //room!.client.updateIosBadge();
    }
  ...
  void dispose() {
    ...
    SocketHelper.shared.socket.off('update', (data) => update());
    SocketHelper.shared.socket.off('logout', (data) => logout());
    super.dispose();
  }
  ...
  Future<void> send() async {
    ...
    GetMessage.send(chat!.id, sendController.text, [], []);
  ...
  void sendFileAction() async {
    ...
    await showDialog(
      ...
      useRootNavigator: false,
      builder: (c) => SendFileDialog(
        file: MFile(name: result.fileName!, path: result.path!),
        room: chat!,
      ),
    );
  }
  ...
  void openCameraAction() async {
    ...
    await showDialog(
      ...
      useRootNavigator: false,
      builder: (c) => SendFileDialog(
        file: MFile(name: file.name, path: file.path),
        room: chat!,
      ),
    );
  }
  ...
  String _getSelectedEventString() {
    ...
    if (selectedEvents.length == 1) {
      return selectedEvents.first.text ?? '';//selectedEvents.first.getDisplayEvent(timeline!).getLocalizedBody(MatrixLocals(L10n.of(context)!));
    }
    for (final event in selectedEvents) {
      if (copyString.isNotEmpty) copyString += '\n\n';
      copyString += event.text ?? '';//"event.getDisplayEvent(timeline!).getLocalizedBody(MatrixLocals(L10n.of(context)!),withSenderNamePrefix: true)";
    }
    return copyString;
  }
  ...
  void redactEventsAction() async {
    final confirmed = await showOkCancelAlertDialog(
        ...
        OkCancelResult.ok;
    if (!confirmed) return;
    GetMessages.del(int.parse(roomId!), [...selectedEvents.map((el) => el.id)]);
    // for (final event in selectedEvents) {
    //   await showFutureLoadingDialog(
    //       context: context,
    //       future: () async {
  ...
  void forwardEventsAction() async {
    Matrix.of(context).shareContent = MMessage.getIds(selectedEvents);
  ...
  void onSelectMessage(MMessage event) {
    //if (!event.redacted) {
    if (true) {
