...
import 'package:cardio_messenger/widgets/permission_slider_dialog.dart';
    ...
    switch (action) {
      ...
      case 'kick':
        if (await _askConfirmation()) {
          await showFutureLoadingDialog(
            context: context,
            future: () => GetParticipants.kick(0, widget.user.id),
          );
          Navigator.of(context, rootNavigator: false).pop();
        }
        break;
      ...
      case 'message':
        final roomIdResult = await showFutureLoadingDialog(
          context: context,
          future: () => GetChat.startDirectChat(widget.user.id),
        );
        if (roomIdResult.error != null) return;
        VRouter.of(widget.outerContext)
            .toSegments(['rooms', roomIdResult.result!.id.toString()]);
        Navigator.of(context, rootNavigator: false).pop();
        break;
      // case 'ignore':
      //   if (await _askConfirmation()) {
