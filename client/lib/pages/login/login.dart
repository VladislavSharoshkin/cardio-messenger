...
import 'package:cardio_messenger/services/get_server_push.dart';
import 'package:cardio_messenger/services/get_token.dart';
...
class LoginController extends State<Login> {
  ...
  void login([_]) async {
    ...
    try {
      ...
      String? pushToken = PushNotification.shared.getToken();
      if (pushToken != null) {
        var serverPush = await GetServerPush.get(pushToken);
        pushToken = serverPush.token;
      }

      var getToken = await GetToken.login(usernameController.text, passwordController.text, pushToken);
      if (getToken.error != null) {
        setState(() => passwordError = getToken.error!.text);
        return setState(() => loading = false);
      }
      StoreHelper.shared.setLogin(getToken.token.token, getToken.token.userId);
