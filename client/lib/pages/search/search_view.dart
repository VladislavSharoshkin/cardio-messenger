...
import 'package:cardio_messenger/pages/chat_list/chat_list_item.dart';
import 'package:cardio_messenger/utils/string_extension.dart';
import 'package:cardio_messenger/widgets/avatar.dart';
import 'package:cardio_messenger/widgets/contacts_list.dart';
                        ...
                        onTap: () async {
                          final roomID = await showFutureLoadingDialog(
                            ...
                            future: () async {
                              ...
                              var getChat = await GetChat.startDirectChat(foundProfile.id);
                              int roomId = getChat.id;
                              return roomId;
                            },
                          );
                          if (roomID.error == null) {
                            VRouter.of(context)
                                .toSegments(['rooms', roomID.result!.toString()]);
                          }
                        },
                        leading: Avatar(
                          mxContent: foundProfile.avatarID?.uri,
                          name: foundProfile.getFullName(),
                          //size: 24,
                        ),
                        title: Text(
                          foundProfile.getFullName(),
                          style: const TextStyle(),
                          maxLines: 1,
                        ),
                        subtitle: Text(
                          foundProfile.username.toString(),
                          maxLines: 1,
                          style: const TextStyle(
                            fontSize: 12,
                          ),
