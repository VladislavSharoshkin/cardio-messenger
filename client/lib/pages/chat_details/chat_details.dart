...
import 'package:cardio_messenger/pages/chat_details/chat_details_view.dart';
...
class ChatDetailsController extends State<ChatDetails> {
  ...
  void setAvatarAction() async {
    ...
    if (PlatformInfos.isMobile) {
      ...
      file = await GetFile.send(result.path);
    } else {
      final result =
    ...
    await showFutureLoadingDialog(
      context: context,
      future: () => GetChat.edit(int.parse(roomId ?? '0'), null, null, file.id)//room!.setAvatar(file),
    );
  }
  ...
  Future<bool> getTimeline() async {
    members ??= await GetParticipants.getParticipants(int.parse(roomId!));
    room = await GetChat.get(int.parse(roomId!));
    return true;
  }
